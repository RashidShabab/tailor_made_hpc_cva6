...
package ariane_pkg;
  ...
  // tmh_event_gen only distinguishes XORL/ORL/ANDL (Boolean) from other ALU

  // ops (Arithmetic). BRANCH/LD/SD are op-level stimulus values used by the

  // linear testbench's make_instr() -- the classifier only ever looks at

  // .fu to decide B/L/S, so the exact op value there doesn't affect

  // classification, but it needs to exist as a legal identifier.
